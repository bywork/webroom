...
export async function generateRTCToken(roomUUID: string): Promise<string> {
    const { token } = await post<GenerateRTCTokenResult>("agora/token/generate/rtc", {
        roomUUID,
    });
    return token;
}
...
export async function generateRTMToken(): Promise<string> {
    const { token } = await post<GenerateRTMTokenResult>("agora/token/generate/rtm", undefined);
    return token;
}
...
/** {@link https://docs.agora.io/cn/cloud-recording/restfulapi/#/云端录制/acquire} 二开标识 云端录制 这里还没换接口*/
export function cloudRecordAcquire(
    payload: CloudRecordAcquirePayload,
): Promise<CloudRecordAcquireResult> {
    return post<CloudRecordAcquireResult>("Agora/cloud_recording/start", {
        room_id: payload.roomUUID,
        ...payload,
...
export function cloudRecordStart(
    ...
    return post<CloudRecordStartResult>("Agora/room/update-status/started", {
        ...payload,
        apiModule: RTMAPI_DOMAIN,
    });
}
...
export function cloudRecordQuery(
    ...
    return post<CloudRecordQueryResult>("Agora/cloud_recording/query", {
        ...payload,
        apiModule: RTMAPI_DOMAIN,
    });
}
...
export function cloudRecordUpdateLayout(
    ...
    return post<CloudRecordUpdateLayoutResult>("Agora/cloud_recording/UpdateLayout", {
        ...payload,
        apiModule: RTMAPI_DOMAIN,
    });
}
...
export function cloudRecordStop(payload: CloudRecordStopPayload): Promise<CloudRecordStopResult> {
    return post<CloudRecordStopResult>("Agora/cloud_recording/stop", {
        ...payload,
        apiModule: RTMAPI_DOMAIN,
    });
}
