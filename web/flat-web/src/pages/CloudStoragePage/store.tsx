...
import { INVITE_BASEURL, STORAGE_DOMAIN } from "../../constants/process";
...
export type CloudStorageFile = CloudStorageFileUI &
    Pick<
        CloudFile,
        "fileURL" | "taskUUID" | "taskToken" | "region" | "task_uuid" | "task_token" | "file_uuid"
    >;

export type FileMenusKey = "download" | "rename" | "delete" | "rememo";
...
export class CloudStorageStore extends CloudStorageStoreBase {
    ...
    public onItemMenuClick = (fileUUID: FileUUID, menuKey: React.Key): void => {
        switch (menuKey) {
            ...
            case "rememo": {
                Modal.confirm({
                    content: <Input placeholder="请输入备注" id="aaa"></Input>,
                    onOk: async () => {
                        const element: HTMLInputElement = document.getElementById(
                            "aaa",
                        ) as HTMLInputElement;
                        try {
                            await this.reFileMemo(fileUUID, element.value);
                            void message.success("备注成功");
    ...
    public reFileMemo = (fileUUID: FileUUID, txt: string): Promise<void> => {
        return fileMemo({
            file_uuid: fileUUID,
            file_memo: txt,
        });
    ...
    /** When a filename is changed to a meaningful new name 二开标识 云盘修改文件名*/
    public onNewFileName = async (
        ...
        if (file) {
            if (file.fileName === fileNameObject.fullName) {
                ...
                await renameFile({ file_uuid: fileUUID, file_name: fileNameObject.fullName });
                runInAction(() => {
            ...
            for (const cloudFile of cloudFiles) {
                ...
                runInAction(() => {
                    if (file) {
                        ...
                        file.convert = CloudStorageStore.mapConvertStep(cloudFile.convert_step);
                        file.taskToken = cloudFile.taskToken;
                        file.taskUUID = cloudFile.taskUUID;
                        file.file_memo = cloudFile.file_memo;
                    } else {
                        this.filesMap.set(
                            cloudFile.fileUUID,
                            observable.object({
                                ...cloudFile,
                                convert: CloudStorageStore.mapConvertStep(cloudFile.convert_step),
                            }),
                        );
                    }
                });
                ...
                if (
                    cloudFile.convert_step === FileConvertStep.Done ||
                    cloudFile.convert_step === FileConvertStep.Failed
                ) {
    ...
    private async removeFiles(fileUUIDs: FileUUID[]): Promise<void> {
        try {
            await removeFiles({ file_uuids: fileUUIDs.join(",") });
            runInAction(() => {
